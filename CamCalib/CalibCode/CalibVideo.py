...
def Transfer():
    ...
    print("外参标定时采用的内参矩阵为:\n",mtx)
    # 载入固定标定图
    images = glob.glob('D:/Users/Desktop/CamCalib/Calibresult/T.jpg')  # 选择需要标定的图片
    if len(images) == 0:
...
def DetectCircle(i, matrix, Rmatrix, tmatrix):
    # 载入图片
    image = glob.glob('D:/Users/Desktop/CamCalib/Calibresult/Circle%d.jpg'%i)
    if len(image) == 0:
        print('No Detect Picture can be loading!')
        exit()
    img = cv2.imread(image[0])

    Imageroi = img[154:661, 299:965]  # roi [y,x]
    ...
    for cnt in contours:
        ...
        if 5000 < area < 55000:
            ...
            circle_point = np.array([Sx + 299, Sy + 154], dtype=np.float32)
            center = (int(Sx + 299), int(Sy + 154))
            radius = int(radius)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if k == ord(' '):
            ...
            # 每次开机都要对外参进行校正
            if not Tflag:
                cv2.imwrite("D:/Users/Desktop/CamCalib/Calibresult/T.jpg", frame)
                print("开始标定外参...")
                Transfer()
                print('获取圆心...请按空格键\n')
                Tflag += 1
                ...
                # 保存关键帧
                cv2.imwrite("D:/Users/Desktop/CamCalib/Calibresult/Circle%d.jpg" % count, frame)
